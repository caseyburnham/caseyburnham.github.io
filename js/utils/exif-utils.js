 ...
 */
function parseExifDate(dateObj) {
	if (!dateObj) return null;
	if (dateObj.year && dateObj.month && dateObj.day) {
		if (!isValidCalendarDate(dateObj.year, dateObj.month, dateObj.day)) return null;
		return new Date(dateObj.year, dateObj.month - 1, dateObj.day, dateObj.hour || 0, dateObj.minute || 0, dateObj.second || 0);
	}
...
export function formatElevation(altitudeMeters) {
	const meters = Number(altitudeMeters);
	if (altitudeMeters == null || !Number.isFinite(meters)) {
		return {
			display: 'Unknown',
			feetRaw: null
		};
	}
	const feetRaw = Math.round(meters * 3.28084);
	return {
